...
tf.flags.DEFINE_string("test_image_dir", "/tmp/test2014",
                       "Validation image directory.")
...
tf.flags.DEFINE_string("test_captions_file", "/tmp/captions_val2014.json",
                       "Validation captions JSON file.")
...
def _process_caption(caption):
  ...
  tokenized_caption.extend(nltk.tokenize.word_tokenize(str(caption).lower()))
...
def _load_and_process_metadata(captions_file, image_dir):
  ...
  id_to_filename = [(x["id"], x["file_name"]) for x in caption_data["image"]]
...
def main(unused_argv):
  ...
  #                                                FLAGS.val_image_dir)
  #added by George Purnell to support Clarity training
  clarity_train_dataset = _load_and_process_metadata(FLAGS.train_captions_file,
                                                    FLAGS.train_image_dir)
  clarity_val_dataset = _load_and_process_metadata(FLAGS.val_captions_file,
                                                  FLAGS.val_image_dir)
  clarity_test_dataset = _load_and_process_metadata(FLAGS.test_captions_file,
  ...
  # Create vocabulary from the training captions.
  train_captions = [c for image in clarity_train_dataset for c in image.captions]
  vocab = _create_vocab(train_captions)

  _process_dataset("train", clarity_train_dataset, vocab, FLAGS.train_shards)
  _process_dataset("val", clarity_val_dataset, vocab, FLAGS.val_shards)
  _process_dataset("test", clarity_test_dataset, vocab, FLAGS.test_shards)
