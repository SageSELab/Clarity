...
def prepro_captions(imgs):
  ...
  for i,img in enumerate(imgs):
    ...
    for j,s in enumerate(img['captions']):
       #try:
       txt = str(s).strip().split() # we no longer need to cast to lowercase or remove punctuation because this is done already by earlier preprocessing scripts #str(s).lower().translate(None, string.punctuation).strip().split()
       img['processed_tokens'].append(txt)
       if i < 10 and j == 0: print(txt)
       #except UnicodeEncodeError:
          #pass;#print("UnicodeEncodeError on: " + s[0:10])
...
def main(params):
  ...
  # assign the splits
  # commented out because splits are already assigned by the update_csv.py script
  #assign_splits(imgs, params) 
  
  
  
  # i.e. each dictionary entry in 'imgs' has a field "split" : "val" or "split" : "test" or "split" : "train"
  ...
  print("Writing out " + params['output_h5'] + " ...")
  ...
  for i,img in enumerate(imgs):
    ...
    refDict = {}
    refDict['path'] = os.path.join(params['images_root'], img['file_path'])
    refDict['refs'] = []
    for cap in img['final_captions']:
      refDict['refs'].append(cap)
		
    out['images'].append(jimg)
    refOut['images'].append(refDict)
  
  json.dump(out, open(params['output_json'], 'w'))
  json.dump(refOut, open(params['ref_path_json'], 'w'))
...
if __name__ == "__main__":
  ...
  #added by ali (so each prepro type has its own paths and refs json)
  
  parser.add_argument('--ref_path_json', required=True, help='Json file containing paths and references for the given preprocessing type. Used for validation')
