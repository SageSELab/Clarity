#Script to transfer learn, fine tune, or learn from scratch for VGG16 from a given custom data split
...
if len(os.sys.argv) < 5:
    print("Retrains an VGG16 model either by transfer learning (changing just the last layer), fine tuning (changing last layer and layers deeper into the model), or by learning from scratch (initializing the VGG16 architecture with random weights).")
...
image_size = 224  #VGG16 has 224 x 224 images as input
...
model = VGG16(weights=(None if from_scratch else 'imagenet'), include_top=False)
...
model.add(VGG16(weights=(None if from_scratch else 'imagenet'), input_tensor=input_layer, include_top=False)) # initialize VGG16 with imagenet weights; use None for randomly initialized weights (from scratch)
...
#Note that layers[0] is the VGG16 model because of the first model.add
...
base_filename = "VGG16-" + str(num_files_wd)
