...
cmd = torch.CmdLine()
cmd:text()
cmd:text('Evaluate an Image Captioning model')
cmd:text()
cmd:text('Options')
...
cmd:option('-dump_json_path', "vis", 'path to the prediction json written out (including filename)')
...
local function eval_split(split, evalopt)
  ...
  while true do
    ...
    local seq1, seq2, seq3 = protos.lm:sample(feats, sample_opts)
    local sents1 = net_utils.decode_sequence(vocab, seq1)
    local sents2 = net_utils.decode_sequence(vocab, seq2)
    local sents3 = net_utils.decode_sequence(vocab, seq3)
    
    for i, curr_sents in pairs({sents1, sents2, sents3}) do
    
        for k=1,#curr_sents do
          local entry = {image_id = data.infos[k].id, caption = curr_sents[k]}
          if opt.dump_path == 1 then
            entry.path = data.infos[k].file_path
          end
          
          -- i.e. caption1, caption2, caption3
          caption_id = "caption"..(tostring(i))
          
          if predictions[data.infos[k].file_path] == nil then
            predictions[data.infos[k].file_path] = {}
            predictions[data.infos[k].file_path][caption_id] = entry.caption
          else
            predictions[data.infos[k].file_path][caption_id] = entry.caption
          end
          ...
          if verbose then
           print(string.format("\"%s\" (%s)\n", entry.caption, data.infos[k].file_path))
  ...
  if opt.language_eval == 1 then
    --the following code was added by Ali to get the prepro type from the model filename (kind of a hack, but this way we can just feed in a model and it detects which preprocessing type was done)
    local s = opt.model:find(".t7")

    local i = s

    while opt.model:sub(i,i) ~= "-" do
       i = i - 1
    end

    local preprotype = opt.model:sub(i + 1, s - 1)

    lang_stats = net_utils.language_eval(0, predictions, opt.id, preprotype)
  end
...
if opt.dump_json == 1 then
  -- dump the json
  utils.write_json(opt.dump_json_path, split_predictions)
end
