...
if len(sys.argv) < 4:
    ...
    print("one with BLEU scores on the test set for each checkpoint, and another with BLEU scores on the validation set for each checkpoint")
    ...
    print("\nusage: python3 " + __file__ + " <directory with im2txt prediction jsons> <whether the predictions come from a model trained on high, low, or combined> <whether these jsons are predictions on the val or test set>")
    print("\nex: python3 " + __file__ + " ./predictions low test\n")
    exit()
global PREDS_DIR
...
GROUND_TRUTH_VAL_PATH = "./coco_caption/ground_truth/%s/captions_val.json"%model_type


# path to the ground truths of the test split (on the given 'model_type')
GROUND_TRUTH_TEST_PATH = "./coco_caption/ground_truth/%s/captions_test.json"%model_type
...
def main_work_function(pred_json_name):
    ...
    for entry in all_preds:
        ...
        append_entry["scores"] = [] # list corresponding to captions
