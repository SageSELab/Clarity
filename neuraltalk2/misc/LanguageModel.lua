...
function layer:sample_beam(imgs, opt)
  ...
  local seq1 = torch.LongTensor(self.seq_length, batch_size):zero()
  local seq2 = torch.LongTensor(self.seq_length, batch_size):zero()
  local seq3 = torch.LongTensor(self.seq_length, batch_size):zero()
  ...
  for k=1,batch_size do
    ...
    seq1[{ {}, k }] = done_beams[1].seq -- the first beam has highest cumulative score
    seq2[{ {}, k }] = done_beams[2].seq -- 2
    seq3[{ {}, k }] = done_beams[3].seq -- 3
    seqLogprobs[{ {}, k }] = done_beams[1].logps
  end

  -- return the samples and their log likelihoods
  return seq1, seq2, seq3 -- seqLogprobs
end
